...
function replayOnce(root, childPaths, owned, sequence, token, image) {
  const name = `sf-pgacl-v2-${process.pid}-${token.slice(0, 12)}-${sequence}`;
  assert(/^[a-z0-9][a-z0-9-]{0,62}$/.test(name), 'REPLAY_V2_CONTAINER_NAME_INVALID');
  ...
  ], undefined, 30_000, 128 * 1024, 'REPLAY_V2_CONTAINER_CREATE_FAILED');
  assert(/^[0-9a-f]{64}\n$/.test(decodeUtf8(output, 'REPLAY_V2_CONTAINER_ID_UTF8_INVALID')),
    'REPLAY_V2_CONTAINER_ID_INVALID');
  ...
  ], undefined, 30_000, 64 * 1024, 'REPLAY_V2_DATABASE_CREATE_FAILED');
  assert(created.byteLength === 0, 'REPLAY_V2_DATABASE_CREATE_OUTPUT_INVALID');
...
`);
  assert(metadata.equals(Buffer.from('sf_public_baseline\tpostgres\tUTF8\tc\tC\tC\tt\n')),
    'REPLAY_V2_DATABASE_CONFIGURATION_INVALID');
  const capture = runChild(root, childPaths.capture, name, 'REPLAY_V2_CAPTURE');
  const oracle = runChild(root, childPaths.oracle, name, 'REPLAY_V2_ORACLE');
  const witness = runChild(root, childPaths.witness, name, 'REPLAY_V2_WITNESS');
  ...
  assert(capture.dataVolumeNameSha256 === volumeHash
    ...
    && witness.dataVolumeNameSha256 === volumeHash, 'REPLAY_V2_VOLUME_EVIDENCE_MISMATCH');
  return { sequence, volumeName: resource.volumeName, capture, oracle, witness };
}

function inspectImage(root) {
  const value = parseDockerSingleton(runOk(root, 'docker', [
    'image', 'inspect', IMAGE_REFERENCE,
  ], undefined, 30_000, MAX_DOCKER_OUTPUT, 'REPLAY_V2_IMAGE_INSPECT_FAILED'),
  'REPLAY_V2_IMAGE_INSPECT_JSON_INVALID');
  ...
  'REPLAY_V2_IMAGE_IDENTITY_INVALID');
  return value;
}
...
function validateContainer(value, resource, image) {
  assert(value.Id === resource.id && value.Name === `/${resource.name}`
    ...
    && noPublishedPorts(value.NetworkSettings?.Ports), 'REPLAY_V2_CONTAINER_IDENTITY_INVALID');
  ...
  'REPLAY_V2_CONTAINER_DEFAULT_COMMAND_INVALID');
  ...
  assert(sameStringBag(value.Config?.Env, expectedEnv),
    'REPLAY_V2_CONTAINER_ENVIRONMENT_INVALID');
  ...
  'REPLAY_V2_CONTAINER_MOUNTS_INVALID');
  const mount = value.Mounts[0];
  assert(mount?.Type === 'volume' && mount.Destination === PGDATA && mount.RW === true
    && /^[0-9a-f]{64}$/.test(mount.Name ?? ''), 'REPLAY_V2_CONTAINER_VOLUME_INVALID');
  ...
  'REPLAY_V2_CONTAINER_ANONYMOUS_VOLUME_INVALID');
}
...
function cleanupOwned(root, owned) {
  ...
  for (const resource of [...owned].reverse()) {
    try {
      ...
      ], undefined, 30_000, 64 * 1024, 'REPLAY_V2_CLEANUP_LIST_FAILED');
      assert(/^[0-9a-f]{12,64}\n$/.test(decodeUtf8(listed,
        'REPLAY_V2_CLEANUP_LIST_UTF8_INVALID')), 'REPLAY_V2_CLEANUP_CONTAINER_MISSING');
      ...
      'REPLAY_V2_CLEANUP_OWNERSHIP_INVALID');
      ...
      'REPLAY_V2_CLEANUP_VOLUME_OWNERSHIP_INVALID');
      resource.volumeName = mounts[0].Name;
      runOk(root, 'docker', ['container', 'rm', '--force', container.Id], undefined,
        30_000, 64 * 1024, 'REPLAY_V2_CLEANUP_CONTAINER_REMOVE_FAILED');
      const volume = runOk(root, 'docker', ['volume', 'rm', resource.volumeName], undefined,
        30_000, 64 * 1024, 'REPLAY_V2_CLEANUP_VOLUME_REMOVE_FAILED');
      assert(decodeUtf8(volume, 'REPLAY_V2_CLEANUP_VOLUME_REMOVE_UTF8_INVALID')
        === `${resource.volumeName}\n`, 'REPLAY_V2_CLEANUP_VOLUME_REMOVE_OUTPUT_INVALID');
    } catch (error) {
...
function runPsql(root, id, input) {
  ...
  ], Buffer.from(input, 'utf8'), 30_000, 128 * 1024, 'REPLAY_V2_PSQL_FAILED');
}

function waitUntilReady(root, id) {
  const deadline = Date.now() + 60_000;
  while (Date.now() < deadline) {
    const result = runRaw(root, 'docker', [
      'exec', id, 'pg_isready', '-q', '-U', 'postgres', '-d', 'postgres',
    ], undefined, 5_000, 64 * 1024, 'REPLAY_V2_READINESS_COMMAND_FAILED');
    assert(result.stdout.byteLength === 0 && result.stderr.byteLength === 0
      && [0, 1, 2, 3].includes(result.status), 'REPLAY_V2_READINESS_OUTPUT_INVALID');
    if (result.status === 0) return;
    Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 250);
  }
  assert(false, 'REPLAY_V2_READINESS_TIMEOUT');
}

function assertContainerNameAbsent(root, name) {
  const output = runOk(root, 'docker', [
    'container', 'ls', '--all', '--filter', `name=^/${name}$`, '--format', '{{.ID}}',
  ], undefined, 30_000, 64 * 1024, 'REPLAY_V2_CONTAINER_PREFLIGHT_FAILED');
  assert(output.byteLength === 0, 'REPLAY_V2_CONTAINER_NAME_COLLISION');
}

function inspectContainer(root, id) {
  return parseDockerSingleton(runOk(root, 'docker', ['container', 'inspect', id], undefined,
    30_000, MAX_DOCKER_OUTPUT, 'REPLAY_V2_CONTAINER_INSPECT_FAILED'),
  'REPLAY_V2_CONTAINER_INSPECT_JSON_INVALID');
}
...
function errorCode(error) {
  return error instanceof Error && /^REPLAY_V2_[A-Z0-9_]+$/.test(error.message)
    ? error.message : 'REPLAY_V2_INTERNAL_ERROR';
}
