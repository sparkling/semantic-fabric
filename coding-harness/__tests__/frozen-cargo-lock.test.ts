...
const PINNED_LOCK_DIGEST = createHash('sha256').update(PINNED_LOCK_CONTENTS).digest('hex');
...
describe('historical frozen Cargo lock preparation', () => {
  ...
  for (const kind of ['structured-offline', 'native-offline'] as const) {
    it(`prepares and validates a baseline lock through ${kind}`, async () => {
      ...
      const lease = await prepareFrozenCargoLock({
        ...
        config,
        executor,
        expectedDigest: PINNED_LOCK_DIGEST,
        pinnedLockfileContents: PINNED_LOCK_CONTENTS,
      });
  ...
  it('installs pinned lock bytes without resolving against the mutable registry index', async () => {
    const fixture = repository();
    const parent = privateRoot('coding-harness-frozen-lock-pinned-');
    const stages: string[] = [];
    const executor: FrozenCargoLockExecutor = {
      kind: 'native-offline',
      execute: async (request) => {
        stages.push(request.command.argv[0] ?? 'missing');
        return attested(request, successfulResult(JSON.stringify({
          workspace_root: request.workspaceRoot,
          target_directory: request.writablePaths[1],
          packages: [],
          workspace_members: [],
          resolve: { nodes: [] },
        })));
    ...
    expect(readFileSync(lease.lockfile.sourcePath, 'utf8')).toBe(PINNED_LOCK_CONTENTS);
    expect(lease.lockfile.digest).toBe(PINNED_LOCK_DIGEST);
    await lease.cleanup();
  });

  it('rejects mismatched pinned lock bytes before running metadata', async () => {
    const fixture = repository();
