...
import {
  ...
  writePrivateReceipt,
} from './programme-v5-operator-support.mjs';

export { parsePolicyReviewReceipt } from './programme-v5-operator-support.mjs';
...
const LAUNCHER_PATH = 'coding-harness/scripts/launch-programme-v5.mjs';
...
export function runProgrammeV5Operator(argv, environment = process.env) {
  ...
  try {
    ...
    if (invocation.operation === 'policy-review') {
      ...
      const parsed = parseExecutionSummary(result.stdout, invocation.expectedPolicyFingerprint);
      assertProgrammeV5ChildStatus('execute', result.status, parsed);
      output = parsed;
    } else {
      output = parseReplaySummary(result.stdout, invocation);
    }
  } catch (error) {
    failure = error;
  }
...
export function programmeV5OperatorExitCode(operation, result) {
  return operation === 'execute' && result?.status !== 'pass' ? 1 : 0;
}

export function assertProgrammeV5ChildStatus(operation, childStatus, result) {
  if (childStatus !== programmeV5OperatorExitCode(operation, result)) {
...
function parseExecutionSummary(serialized, expectedPolicyFingerprint) {
  let value;
  ...
  exactKeys(value, [
    'status', 'reason', 'receiptDigest', 'programmeAcceptanceDigest', 'envelopeDigest',
...
function parseReplaySummary(serialized, invocation) {
  let value;
  try { value = JSON.parse(serialized); }
  ...
  exactKeys(value, [
    'verificationStatus', 'recordedStatus', 'recordedReason', 'receiptPath',
    'replayReceiptDigest', 'envelopeDigest', 'policyFingerprint', 'launchReceiptDigest',
...
if (process.argv[1] !== undefined && realpathSync(process.argv[1]) === scriptPath) {
  try {
    const result = runProgrammeV5Operator(process.argv.slice(2));
    process.stdout.write(`${JSON.stringify(result)}\n`);
    process.exitCode = programmeV5OperatorExitCode(process.argv[2], result);
  } catch (error) {
