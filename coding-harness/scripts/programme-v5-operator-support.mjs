...
export function parsePolicyReviewReceipt(serialized, expected) {
  ...
  const exactSerialized = serialized.endsWith('\n') ? serialized.slice(0, -1) : serialized;
  if (JSON.stringify(value) !== exactSerialized) {
    throw new Error('HARNESS_OPERATOR_POLICY_RECEIPT_INVALID');
  }
  ...
  if (value.schemaVersion !== 1 || value.authority !== 'development-only-no-promotion'
    || value.operation !== 'programme-v5-policy-review'
...
function policyReceiptBindingsMatch(receipt, policy) {
  ...
  return bootstrap?.controllerStoreDigest === receipt.controllerStoreDigest
    ...
    && policyRunIdMatches(policy, receipt.runId);
}
