...
export function normalizeNativeReviewError(error: unknown): unknown {
  return failureCodeForError(error) === GENERIC
    ? new Error('HARNESS_NATIVE_REVIEW_FAILED', { cause: error })
    : error;
}
