...
export const PROGRAMME_V5_CLAIM_AUTHORITY_ROOT =
  '/home/claude/.semantic-fabric-harness';

export function programmeV5ArtifactPath(
  repositoryRoot: string,
  runId: string,
  suffix: 'policy-review' | 'execution-claim' | 'execution' | 'replay',
): string {
  if (!/^[A-Za-z0-9_-]{8,160}$/.test(runId)) {
    throw new Error('HARNESS_PROGRAMME_V5_ARTIFACT_RUN_ID_INVALID');
  }
...
export function requireProgrammeV5ArtifactPath(
  repositoryRoot: string,
  runId: string,
  suffix: 'policy-review' | 'execution-claim' | 'execution' | 'replay',
  value: string,
): string {
  const expected = programmeV5ArtifactPath(repositoryRoot, runId, suffix);
  if (!isAbsolute(value) || resolve(value) !== value || value !== expected || value.includes('\0')) {
    throw new Error('HARNESS_PROGRAMME_V5_ARTIFACT_PATH_INVALID');
  }
  return value;
}

export function readProgrammeV5PrivateArtifact(
  repositoryRoot: string,
  path: string,
  maximumBytes = MAX_ARTIFACT_BYTES,
): string {
  const runs = prepareRunsDirectory(repositoryRoot, false);
  return readPrivateArtifact(runs, path, maximumBytes);
}

export function programmeV5AuthorityClaimPath(
  ...
  if (!/^[a-f0-9]{64}$/.test(claimKeyDigest) || claimKeyDigest === '0'.repeat(64)) {
    throw new Error('HARNESS_PROGRAMME_V5_CLAIM_KEY_INVALID');
  }
  return join(authorityRootPath(authorityRoot), 'programme-v5-claims', `${claimKeyDigest}.json`);
}

export function readProgrammeV5AuthorityClaim(
  path: string,
  authorityRoot = PROGRAMME_V5_CLAIM_AUTHORITY_ROOT,
...
export function writeProgrammeV5AuthorityClaim(
  path: string,
  contents: string,
  authorityRoot = PROGRAMME_V5_CLAIM_AUTHORITY_ROOT,
  maximumBytes = 100_000,
  ...
  writePrivateArtifact(
    prepareClaimDirectory(authorityRoot, true), path, contents, maximumBytes,
  );
}
...
function readPrivateArtifact(directoryPath: string, path: string, maximumBytes: number): string {
  ...
  try {
    ...
    try {
      ...
      if (!sameFile(pathStat, before)) throw new Error('HARNESS_PROGRAMME_V5_ARTIFACT_CHANGED');
      const bytes = Buffer.allocUnsafe(Number(before.size));
      let offset = 0;
      while (offset < bytes.length) {
        const count = readSync(descriptor, bytes, offset, bytes.length - offset, offset);
        if (count === 0) throw new Error('HARNESS_PROGRAMME_V5_ARTIFACT_CHANGED');
        offset += count;
      }
      const after = fstatSync(descriptor, { bigint: true });
      if (!sameFile(before, after)
        || !sameFile(after, lstatSync(anchoredPath, { bigint: true }))) {
        throw new Error('HARNESS_PROGRAMME_V5_ARTIFACT_CHANGED');
      }
      assertDirectoryIdentity(directoryPath, directoryIdentity, directory);
      try { return new TextDecoder('utf-8', { fatal: true }).decode(bytes); }
      catch { throw new Error('HARNESS_PROGRAMME_V5_ARTIFACT_UTF8_INVALID'); }
    } finally { closeSync(descriptor); }
  } finally { closeSync(directory); }
}

export function writeProgrammeV5PrivateArtifact(
  repositoryRoot: string,
  path: string,
  contents: string,
  maximumBytes = MAX_ARTIFACT_BYTES,
): void {
  const runs = prepareRunsDirectory(repositoryRoot, true);
  writePrivateArtifact(runs, path, contents, maximumBytes);
}
...
function writePrivateArtifact(
  ...
  if (bytes.length < 1 || bytes.length > maximumBytes) {
    throw new Error('HARNESS_PROGRAMME_V5_ARTIFACT_SIZE_INVALID');
  }
  const directory = openDirectory(directoryPath);
  try {
    const directoryIdentity = fstatSync(directory, { bigint: true });
...
export function assertProgrammeV5ArtifactAbsent(path: string, error: string): void {
  try { lstatSync(path); }
  catch (caught) {
...
function prepareClaimDirectory(authorityRoot: string, create: boolean): string {
  ...
  return exactDirectory(join(root, 'programme-v5-claims'), create, 'CLAIM_AUTHORITY');
}

function authorityRootPath(value: string): string {
  if (!isAbsolute(value) || resolve(value) !== value || value.includes('\0')) {
    throw new Error('HARNESS_PROGRAMME_V5_CLAIM_AUTHORITY_ROOT_INVALID');
  }
  return value;
}
...
function trustedFileStat(path: string, maximumBytes: number) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || stat.uid !== BigInt(process.getuid?.() ?? 0)
    ...
    throw new Error('HARNESS_PROGRAMME_V5_ARTIFACT_INVALID');
  }
  return stat;
}
...
function assertAbsent(path: string): void {
  ...
  throw new Error('HARNESS_PROGRAMME_V5_RECEIPT_EXISTS');
}
...
function assertDirectoryIdentity(path: string, expected: BigIntStats, descriptor?: number): void {
  ...
  if (!sameDirectory(expected, current)
    ...
    throw new Error('HARNESS_PROGRAMME_V5_RESULTS_ROOT_CHANGED');
  }
}
