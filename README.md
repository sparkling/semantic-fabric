...
- **Current:** Pre-response HTTP failures and startup CLI surfaces are redacted; SELECT/CONSTRUCT executor failures expose only a stable body error. Correlation IDs have no telemetry sink, and post-200 failures cannot become atomic RFC 9457 responses.
- **Current:** The serving lane carries one control identity from ingress through handler handoff and serialization, with one absolute deadline and inclusive ceilings for observable metadata probes, branch opens, pull attempts, semantic result items and serializer writes. Charges and terminal transitions are linearizable: each charge completes wholly before a later terminal transition or observes the sticky existing terminal and leaves counters unchanged. The absolute deadline is rechecked at handoff; non-deadline SELECT/CONSTRUCT failures remain stable post-200 body errors. ASK rejects zero result-item capacity before backend match/acquisition and stops after its first final solution without truncating grouped inner input. Compiler CPU, database rows scanned, recursive SQL iterations/source cost, raw/conformance callers, a common source-native statement-cancellation contract and atomic/no-prefix post-200 responses are not claimed.
...
- **Current:** Serving rejects known source-sized Rust fallback plans, including exact reconstructed-term dedup over complete pre-projection BGP keys, before backend selection or source I/O; raw/conformance execution remains available and bounded physical serving implementations remain planned.
...
JSON-LD. One finite serving control spans the absolute deadline, compiler/pool waits, observable probe/open/pull work, semantic results, and serializer writes; compiler CPU, database rows/recursive SQL/source cost, raw/conformance governance, a common source-native statement-cancellation contract, and atomic/no-prefix post-`200` delivery remain open under [ADR-0010](docs/adr/ADR-0010-security-and-resource-governance.md).
