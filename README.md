...
- **Limitation:** Structural/type schema drift and Direct Mapping regeneration, total-resource-qualified recursive paths, bounded global operators, federation, total request governance, security/identity, observability/lifecycle and an exact production artifact remain planned.
...
- **Qualified:** SQLite, PostgreSQL and MySQL source selectors feed one immutable source/backend/compiler/cache binding; reachability is not admission. PostgreSQL introspection and unqualified base-table resolution are `public`-scoped and fail on schema/catalogue identity ambiguity. Explicit-R2RML serving quarantines unverified integrity constraints before compilation; trusted raw `rr:sqlQuery` is not schema-confined, while structural/type drift, atomic reload and Direct Mapping lifecycle remain open.
...
Commit `24a0e20` closes the stale integrity-constraint rewrite hazard for the authored-R2RML serving lane. Raw catalogue observations must first become a typed, `Unverified` `CompilerSchema`; PK, UNIQUE, FK, functional-dependency and NOT-NULL proofs are removed before compiler/cache construction. Non-vacuous SQLite controls show wrong answers from trusted stale PK, UNIQUE, FK/PK and FD metadata; a separate sanitization assertion covers NOT-NULL removal. Fresh and cached quarantined plans remain correct. The conservative fallback can add DISTINCT work or retain sound `501` boundaries. It does not solve structural/type DDL drift, atomic reload, Direct Mapping regeneration or a future race-safe verified-constraint mode, and grants no backend admission.
...
| Federation | Cross-RDBMS planning is in scope but not implemented. Source affinity, neutral schema DTOs and an enforcing immutable single-source backend/compiler/cache binding exist; authored-R2RML serving quarantines unverified integrity constraints. Stable structural/type snapshot digests, Direct Mapping lifecycle, drift/reload, registry, federated plan and coordinator do not. External SPARQL `SERVICE` remains excluded |
