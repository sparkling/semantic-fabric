...
- The 2026-08-26 closeout passed format, clippy with warnings denied, all-target
  ...
  unexpected failures.
- The versioned engineering harness passes 892 tests across 121 files; two
...
The protected test-only fixture path now snapshots exact intrinsic bytes, pre-scans duplicate decoded keys and allocation ceilings, performs one closed parse/canonical replay, and retains its bytes behind an opaque non-authorizing brand. Its 92 hostile/limit/private-brand KATs plus five exact-fixture baseline tests pass on exact Node 20.0.0 and 24.14.1. An additive pure lexical mutator, extracted fail-closed replay support and 17 KATs identify the projection's exact eight top-level `UNION ALL` class branches and construct four scanner-positioned record-set mutants without changing the pinned source. Its live verifier seeds PUBLIC `USAGE` for one foreign-data wrapper and server inside one serializable transaction, proves all eight classes are populated, matches both the original and explicit-column normalized projection to the independent raw-catalogue bag, and executes all 12 mutants. Every branch deletion preserves the exact seven-class survivor bag; return-zero, one-atom omission, one typed-sentinel addition and count-neutral substitution match mutation-specific exact outputs, with the sentinel independently pinned by a KAT. The full oracle rejects the first 11 with `ORACLE_RECORD_BAG_KEYS_MISMATCH` and the substitution with `ORACLE_RECORD_BAG_MULTIPLICITY_MISMATCH`; rollback, postflight absence, component-walked held-descriptor `O_NOFOLLOW|O_NONBLOCK` source confinement, exact frozen process arguments, classified timeout/output failures, mutated container-isolation predicates, distinct anonymous volumes, network isolation, cleanup and two-run determinism are checked. Session/probe/inspect ceilings are 60/15/15 seconds beneath the parent's 300-second child ceiling, leaving 180 seconds of headroom, while CI's 120-minute ceiling exceeds the conservative 110-minute six-run/cleanup sum. Unpublished developer-local exact Node 20.0.0 and 24.14.1 verification reports V1, V2 and the 12-mutant live replay, 602 supervisor tests, and 892 parent-harness tests with two intentional skips on each runtime. The parent harness also replaces recursive watching with explicit root and discovered-directory watches, an 8,192-watcher fail-closed ceiling, complete partial cleanup, and setup/cooperative/settled digest agreement; digest bytes stay unchanged. These run results have no version-bound repository receipt. The delete-any-top-level-class-branch, return-zero, one-atom omission/addition and count-neutral-substitution subsets are closed locally; predicate/value/nullability/order/duplicate/array, `UNION ALL` replacement and the other ADR-0047 §7 mutations, passing GitHub-hosted evidence, the runtime `Plan` and live-observation brands, and runtime admission remain open. The private artifact stays byte-identical at `e296eabc2480d10b5bcb1ea8629e1855f82a1695891a0e96958de2b81e65b358`; the public bundle remains `90e21e7c0e3a45b66da55f0e8cf9c0a23b3fb82e805223922d81096e097f7c3a`. The ignored, untracked local two-task diagnostic suite verifies structurally at task hash `938531ad…a95` but is ineligible for evolution; npm audits are clean. Ruflo's deep generic scan reports 12 high heuristics: three negative command-injection fixtures/assertions and nine static SQL/error-string patterns; line-level review found no untrusted command or SQL execution in this tranche. The repository-root upstream MetaHarness composite emits clean/info against the tracked posture file, but ADR-0037 treats it as inconclusive because the scanner does not inspect canonical `.mcp.json` or enforce runner isolation. After removing two ignored phantom `coding-harness@latest` registrations and their unused grants from developer-local child settings, the child composite emits `mcp-disabled/info`; this host-local gitignored cleanup is not promotion evidence. Canonical `.mcp.json` still has floating, unpinned, or network-capable coordination entries and the private Ruflo runner still uses `--share-net`; the project-owned configuration and runtime gates must close those separate M0 gaps.
...
| M0 performance evidence | ADR-0041 proposes a separate manifest-bound, single-attempt capture transaction. Exact input attestation, a negative-only host gate, same-UID claim/rooted source, signed registration/checkpoint verification, RFC 9162 proof replay, a sealed exact-row PostgreSQL materializer, and the reviewed exact catalogue plus private parser/digest verifier exist. ADR-0047 now records an independently reproduced test-only PostgreSQL 16.15 PUBLIC-ACL candidate, independent raw-SQL cross-check, immutable V1 receipt, additive V2 OID/attribute-aware candidate-matrix/effective-privilege witness receipt and owned two-run fresh-`template0` replay. Its protected test-only scanner/reader and opaque non-authorizing fixture brand pass 97 focused KATs on exact Node 20/24. An additive 17-KAT mutator/replay-support tranche kills all eight top-level class-branch deletions plus return-zero, one-atom omission/addition and count-neutral substitution in two fresh networkless exact-Node-20/24 runs while leaving the frozen fixture, receipts and artifact unchanged. No version-bound run receipt or passing GitHub-hosted evidence is carried by this change; broader predicate/value/order/nullability/duplicate/array and `UNION ALL` mutation closure, runtime `Plan` and live-observation brands, and admission remain open. All remain test-only and nonauthorizing: they prove no migration, provisioning, capture, performance or release authority. There is still no migration SQL/manifest, driver-free live provisioning verifier, database adapter or migration runtime, independently administered log/service, positive runner admission, controlled performance profile, lease/build/attempt/capture authority, two-builder-agreed artifact, performance baseline, or measured number; this host is ineligible. Next are the remaining pre-sealing mutations and hosted evidence, then ADR-0046's immutable bundle, the runner/live verifier chain and dormant adapter/evidence before external append-only registration/lease authority and controlled-runner admission |
