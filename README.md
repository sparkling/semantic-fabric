...
The protected test-only fixture path snapshots exact intrinsic bytes, enforces decoded-key/allocation ceilings before one closed parse and canonical replay, and retains a private non-authorizing copy; its 97 hostile/limit/private-brand/baseline KATs pass on exact Node 20.0.0 and 24.14.1. The first protected mutator/replay tranche freezes eight top-level `UNION ALL` branch deletions and four record-set mutants; two fresh networkless runs execute and kill all 12 against the independent raw-catalogue bag. A second protected quartet pins all 19 final-`WHERE` deletion spans and mutant digests to the exact 6,859-byte projection. Two serializable rollback-only batches of ten and nine derive their expected bag before adding 15 independently counted hidden predicate witnesses, require the unchanged projection to remain equal, and execute every mutant through a separate loose closed eight-field decoder. The fixed result is **19 executed; 15/15 non-equivalent killed; four independently proved guard-equivalent; zero unresolved**. Two owned runs reproduce 11,963,849-byte (`2df643f9…c50f`) and 11,608,234-byte (`688a5ed3…522`) transcripts, complete rollback, postflight absence, distinct anonymous volumes, network isolation and cleanup. CI now runs all eight isolated V1/V2/branch/final-`WHERE` replays on exact Node 20/24 under a 150-minute ceiling for the conservative 140-minute sum. Current service verification is 611/611; the parent harness retains explicit bounded directory watches and setup/cooperative/settled digest agreement. No version-bound run receipt or passing hosted evidence is carried. Branch, record-set and final-`WHERE` subsets are closed locally; CASE/JOIN/value/nullability/order/duplicate/array/element and `UNION ALL` replacements, the runtime `Plan`, live-observation brands and admission remain open. The private artifact stays `e296eabc2480d10b5bcb1ea8629e1855f82a1695891a0e96958de2b81e65b358` and the public bundle `90e21e7c0e3a45b66da55f0e8cf9c0a23b3fb82e805223922d81096e097f7c3a`. Ruflo's generic scans remain `INCONCLUSIVE` because they do not inspect the tracked launchers. The project gate requires exact-empty `.mcp.json`, an exact `.agents/config.toml` digest, and a pinned networkless two-status-file reader; fixed-seed Darwin Shield remains 9/12 and grants no promotion. The retrieval-policy flywheel remains off.
