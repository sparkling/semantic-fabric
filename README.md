...
As of 2026-09-02, the generated catalog records:
...
- **Limitation:** A coherent structural/type schema generation with typed synthetic row identity, atomic reload and Direct Mapping regeneration, total-resource-qualified recursive paths, bounded global operators, federation, total request governance, security/identity, observability/lifecycle and an exact production artifact remain planned.
...
- **Current:** Serving rejects known source-sized Rust fallback plans, including exact reconstructed-term dedup over complete pre-projection BGP keys, before backend selection or source I/O. Top-level ordered ASK is admitted without a global sort buffer; other source-sized states and ordered nested subplans still reject. Raw/conformance execution remains available and bounded physical serving implementations remain planned.
- **Current:** SQLite has required evidence for a strict read-only endpoint subset: exactly one query in GET/form POST or one raw POST body; raw per-request cap n, checked form wire cap 3n+16 and decoded cap n; optional version/dataset/extra parameters rejected instead of ignored; unsupported media rejected without application-body polling; ASK JSON, SELECT JSON/XML/CSV/TSV and CONSTRUCT Turtle/N-Triples/JSON-LD. Full Protocol remains open.
...
Optional flags include `--ontology`, `--bind`, `--timeout-secs`, `--max-query-len`,
`--max-source-work`, `--max-result-items`, `--max-serialized-bytes`, and the PostgreSQL/SQLite pool-sizing flags. The default endpoint is
...
SELECT/ASK returns SPARQL Results JSON/XML/CSV/TSV; CONSTRUCT/DESCRIBE returns Turtle/N-Triples/JSON-LD. Tower `Service::call` mints one deadline after HTTP/request-target parsing and before Axum route/method dispatch, then spans admission, compiler/pool waits, observable probe/open/pull work, semantic results, and serializer writes.
Full Protocol, compiler CPU, database rows/recursive SQL/source cost, raw/conformance governance, common source-native statement cancellation, and atomic/no-prefix post-`200` delivery remain open under [ADR-0010](docs/adr/ADR-0010-security-and-resource-governance.md).
...
- SQLite and PostgreSQL runners consume that inventory in canonical order;
  ...
  80/1/6. The 2026-09-02 regeneration preserves aggregate counts and `passed`
  status while reclassifying the typed outcome cause for SQLite `R2RMLTC0002g`
  and PostgreSQL `R2RMLTC0002g`/`R2RMLTC0002h` from `execution-error` to
  `source-validation-error`; SQLite `R2RMLTC0002h` was already
  `source-validation-error`. This is mapping evidence, not runner/toolchain/
  host/provider provenance, SPARQL Query/Protocol conformance, or admission.
...
- The 2026-08-26 closeout passed format, clippy with warnings denied, all-target
  ...
  unexpected failures.
- The 2026-09-02 current-tree hermetic engineering-harness checkpoint passed 966 tests across 124 files with two intentional skips after refreshing the expanded Rust source closure and controller attestation.
...
82 evidence-scoped cells—48 implemented, 32 planned and two unsupported—with zero production-admitted backends; backend-aware
...
exact skips; its file/outcome/inventory digests are `6eff3626…e2818`,
`9ec4a308…f4ca5` and `4d2eb56e…c96`. It closes only the current required-live M0
...
| Federation | Cross-RDBMS planning is in scope but not implemented. Source affinity, neutral schema DTOs and an enforcing immutable single-source backend/compiler/cache binding exist; authored-R2RML serving quarantines unverified constraints/type authority and referenced base Table/Query columns fail before cursor I/O. Offline/synthetic derived-alias folding remains a non-authoritative lexical heuristic. Stable structural/type generations and leases, Direct Mapping lifecycle, drift/reload, registry, federated plan and coordinator do not. External SPARQL `SERVICE` remains excluded |
...
| Exactness and boundedness | Recursive closures now use exact finite-pair fixed points on evidenced dialects and reject unproved dialects; known source-sized ORDER/GROUP/DISTINCT/CONSTRUCT fallbacks reject before I/O. Serving bounds observable probe/open/pull, semantic-result and serializer-byte work; bounded physical alternatives, total compiler/database/recursive/source-cost governance, a common source-native statement-cancellation contract and atomic/no-prefix post-`200` response remain release blockers in ADR-0038 |
...
| `sf-serve` | Read-only SPARQL query HTTP endpoint with finite serving-lane deadline/source/result/byte controls |
| `sf-cli` | `serve`, `conformance`, and `bench` binary |

## Architecture decisions

The canonical [ADR corpus](docs/adr/) contains 47 records: 35 accepted, eleven
...
| Remediation, engineering control plane, application completion and design locks | [ADR-0036](docs/adr/ADR-0036-correctness-first-open-issue-remediation.md)–[ADR-0050](docs/adr/ADR-0050-verified-source-generation-leases-schema-identity-and-atomic-runtime-activation.md) |
