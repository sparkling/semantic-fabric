...
- `RuntimeSnapshot`: immutable ontology, mappings, structural/type source schemas,
  constraint authorities, epochs, and digests, validated before atomic activation;
...
- `FederatedPlan`: per-source SQL fragments, typed global operators, reducers, failure policy, and immutable compile provenance only: runtime/configuration
  epoch plus ontology, mapping, schema, constraint-policy, and capability digests; and
...
### 7. Implementation status (through 2026-09-02)
...
adds the durable required-live PostgreSQL baseline: 80 pass, one documented deviation, and six exact skips across all 87 ordered cases. The 2026-09-02
regeneration preserves aggregate counts and `passed` status while reclassifying the typed outcome cause for SQLite `R2RMLTC0002g` and PostgreSQL
`R2RMLTC0002g`/`R2RMLTC0002h` from `execution-error` to `source-validation-error`; SQLite `R2RMLTC0002h` was already `source-validation-error`.
Current
PostgreSQL receipt/outcome digests are `6eff3626f38b0e9f1f01f21163ae9a2321f494e057b3325a6577a54cbd5e2818` and `9ec4a308f27a858f86a96fdd627b3ba820acbae8fd67aab466d0ba78a08f4ca5`;
SQLite receipt/outcome digests are `e63116087b6c805dd9053bb933f3f258533901b255a363dfc0b8f74313e8550b` and `75ceab034c30af4f02f5e627b7122ec5dbc982a6569f747f9fc3bfe1809d3a41`.
The inventory digest remains `4d2eb56e25920c4b8488b47d971cf887902b4e4178257420bf3a7ea44c504c96`.
Required-live replay fails on provider absence, CI checks but cannot regenerate,
and receipts attest sealed mapping inputs/outcomes—not runner, toolchain, host,
provider, Query/Protocol conformance, or production admission.
...
The public test-only seed is never a runtime key. The fixed SPKI and envelope
traverse the sealed Node-oracle verifier path; digest-domain substitutions and
payload/signature mutations fail. This is not a production verifier.
At that historical checkpoint, the deterministic controller receipt was `97ca5d7…c36606f`; 103 harness files passed (742 tests, two expected skips).
...
Commit `481f870` admits top-level ordered ASK without an unused global sort buffer; genuinely source-sized states and ordered nested subplans remain fail-closed. Commit `1e85249` starts the absolute deadline in Tower `Service::call`, after HTTP/request-target parsing and before Axum route/method dispatch. Commit `36488d5` adds strict request admission: exactly one GET/form query or one raw query; per-request raw cap `n`, checked form wire cap `3n+16` and decoded cap `n`; optional version/dataset/extra parameters reject; unsupported media is rejected without application-body polling. Full Protocol and total QueryBudget remain open.

The 2026-08-31 integrated M0 hardening candidate protects default-empty project MCP launchers and a pinned local Ruflo reader; its opaque historical overlay and private status snapshots add no product authority. The Node/MetaHarness tree remains development/evidence infrastructure. The native reader rejects the locally patched ambient Ruflo source, but may consume an explicit canonical package root only after the same 1,552-file materialized closure and aggregate digest/count/byte pins pass; that locator is not forwarded into its networkless sandbox. Relocated captures now emit schema V3 with a content-addressed source policy and the private executed path; V2 keeps its original physical-source meaning for strict historical replay. The 2026-09-02 current-tree hermetic checkpoint passes 966 tests with two intentional skips across 124 files after refreshing the expanded Rust source closure and controller attestation. Its seven-test mutable ambient Ruflo collector is outside that checkpoint: three fail-closed controls pass and four positive cases reject the installed package as `HARNESS_PROGRAMME_V5_RUFLO_PACKAGE_SOURCE_MISMATCH`; no ambient result grants authority, and Cargo/link lanes remain serialized on this host. OIA remains inconclusive, fixed-seed Darwin Shield passes only 9/12 gates, and the retrieval flywheel stays off. On 2026-09-01, `1e2d88d`/`7d5af51`/`0d5d09e`/`e37cce7` sealed dependency-free Node-oracle contracts; ADR-0042–0047 stay proposed and no deployable Node adapter, runner or live authority may be added. Rust commits `5c379f6`/`639134d` close silent recursive-path prefixes and reject known source-sized fallback plans; `3e0f920`/`c9e6c53` add the initial redacted RFC 9457 boundary; `6cd85eb` establishes one absolute ingress deadline; `3b68ca4`/`6a51bf0`/`58cca82` add atomic accounting, observable executor charging and one governed serving identity; `b3bf6c9` rejects impossible zero-result ASK budgets before executor I/O; `bac8240` rejects unrepresentable timeouts as redacted startup configuration; `087e7e2` linearizes charge/terminal transitions; `f1f4747` rechecks deadline/cancel handoff and preflights zero-capacity ASK before backend acquisition; `136f21b` stops ASK after its first final solution without truncating grouped input or losing ordered slicing; `d391927` completes current router path/method/body rejection mapping and makes expired representable handoffs public `504` independently of sticky internal first-cause accounting; `1e85249` moves deadline creation before Axum route/method dispatch; `36488d5` adds strict media-specific request admission; and `484a4b4` adds a narrow `SourceRef` boundary while leaving TLS, layered configuration and telemetry open. Malformed request targets rejected before the outer Tower service remain outside the application boundary. This is a serving-lane slice, not total QueryBudget or full Protocol: compiler CPU, database rows scanned, recursive SQL iterations/source cost, raw/conformance governance, a common source-native statement-cancellation contract and atomic/no-prefix post-`200` responses remain open. `0464293` adds bounded snapshot-local `SourceId`/`SourceMapping`; `9d228dd` moves neutral schema DTOs to `sf-core`; `67a779a` centralizes compiler capabilities; and `faee07a` binds one source, backend, dialect, T-box, observed schema and cache, rejecting detached plans before I/O. `9d0da85` makes PostgreSQL catalogue observation a coherent read-only repeatable-read `public` snapshot and pins/recycles/verifies the matching unqualified base-relation resolution scope; the follow-up guard rejects `public` names shadowed by `pg_catalog`. Trusted raw `rr:sqlQuery` remains schema-unconfined. Commit `24a0e20` puts an explicit `ConstraintAuthority::Unverified` `CompilerSchema` at the serving boundary, removes PK, UNIQUE, FK, functional-dependency and NOT-NULL claims before compilation, and binds authority into the cache scope; cached and fresh serving plans therefore cannot use later-stale constraints, and duplicate safety stays conservative. Frozen-schema/direct translation tests may still exercise optimiser capability, but current `sf-serve` accepts authored R2RML and does not generate Direct Mapping; because PK/FK facts change Direct Mapping semantics, any future live generator needs a verified generation bound through streamed execution. This closes the constraint-staleness P0, not the schema lifecycle: the pure Phase 1 digest kernel is not adapter-emitted or runtime-bound, and there is still no watcher or atomic reload, and a future verified-constraint mode requires a race-safe lease through the streamed cursor. The 2026-09-02 hardening adds `ColumnTypeAuthority::Unverified`, so cached serving cannot use mutable startup type strings to authorize cross-column PostgreSQL pooling; missing type facts fail closed, while raw translation retains its explicit frozen-schema contract. Execution now recursively probes base Table/Query sources, overlays those live catalogs for base-source references during nested SubPlan emission, rejects missing, duplicate or ambiguous result columns, allocates derived aliases above aliases hidden in nested IQ/SQL conditions, pre-emits every branch before opening the first cursor, propagates SQLite and PostgreSQL conformance `rr:sqlQuery` metadata failures, and redacts post-commit SELECT/CONSTRUCT executor text to one stable body error. Offline/synthetic derived aliases and translate-time immediate wrappers retain a bounded, non-SQL-token-aware lexical heuristic, never live metadata authority. When cross-column compatibility is unproved, each arm captures its full BGP-boundary key, including an active graph variable, before projection; post-cascade ownership remaps it only through physically key-preserving pure unary SubPlan chains, runtime repeats the shape/key proof before metadata I/O, and execution overlays the key only on its private branch clone. Joined/OPTIONAL/path/aggregate, modifier-bearing or multi-branch nested wrappers, key-dropping nested projections, orphaned/multiply owned markers, or groups with fewer than two executable arms return `501`; fully ground overlapping arms use an exact SQL unit-relation pool. The remaining exact reconstructed-term shared set is a source-sized raw/conformance fallback that serving rejects pre-I/O. Compiler-only path controls show the synthetic no-primary-key Direct-Mapping `rowid` rendered as SQLite `rowid` or PostgreSQL `(ctid)::text` at a base table while a D1 Table→Query wrapper preserves the logical name. A real PostgreSQL `rowid` collision, snapshot-local `ctid`, and absent live PostgreSQL path execution keep this non-authoritative. These checks are bounded by mapping/plan size but remain sequential, non-atomic and TOCTOU-prone; SQLite metadata preparation is non-cancellable blocking work. They are defense in depth, not adapter-emitted structural/type identity, a drift watcher, readiness transition or streamed execution lease. The 2026-09-01 graph-scope slice closes the separate generated-blank-node blocker for the evidenced single-source profile: one IQ identity carries `(effective target graph, generated identifier)` through subject, direct/reference object, class, path, dump, query and subplan paths; row-derived `rr:defaultGraph` normalizes to default; and CONSTRUCT template nodes retain a disjoint fresh domain. Differently shaped same-value identifier recipes, dynamic-graph paths and row-dependent rendered-width pooling remain sound `501` boundaries. PostgreSQL/MySQL direct matrices and globally bounded graph dedup remain open. Typed synthetic row identity, runtime-bound snapshot digests, adapter schema generation, atomic reload, registry, federation and production admission remain open. `13b8187`/`8c6181b`/`9b60dc2` seal 139 gold artifacts, 171 files from exact source revision `7c45292fccb8b88afe263e18de6806667ae18573`, and the narrow live Style differential. `cbb63ab` adds a sealed 11-store/112-table/598-column table-and-column inventory plus one read-only repeatable-read transaction per database; the 2026-09-01 live gate failed closed on five unexpected infrastructure tables/45 columns in each of ten populated databases, empty `Style360` (3 tables/21 columns missing), and one changed `ProductDesign` column. It does not cover keys, constraints, indexes, views or privileges, infer mappings, claim a globally atomic snapshot, mutate a database, or establish image/source provenance, production admission or release authority. ADR-0048 remains controlling; `89a97a7`/`a86af7d` only decompose characterized Rust hotspots.
## Acceptance
...
- Good: the existing semantic investment survives; work is sequenced by risk and executable proof rather than by feature count.
