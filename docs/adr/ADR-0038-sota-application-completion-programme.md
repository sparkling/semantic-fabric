...
The canonical revision-pinned product-mock gold (`expected-ontology.json`, 14-category
`categories/`, and `candidate-manifest.json` in `semantic-builder`) and narrow qualified ProductDesign/Style live vertical are evidence only. A Rust table/column inventory gate recounts
