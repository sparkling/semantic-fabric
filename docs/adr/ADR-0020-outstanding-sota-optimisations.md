...
* Good — the non-blocking SOTA surface is recorded with dispositions + design seams; nothing is forgotten, and deferred items have reserved seams so they bolt on without rework.
* Good — two items (cost-driven cross-source semi-join, the plan cache) are high-ROI and already implied by ADR-0006/0007, so they land naturally with the engine.
* Neutral — most items are profile- or requirement-gated; this is a roadmap, not a build order beyond "rewriter core first."
