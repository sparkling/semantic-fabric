...
**Decision:** target the **plugin-pinned 0.3.1** as the supported, ruflo-tested, graceful-degradation-honouring path. To use 0.7.1, invoke `npx -y -p @metaharness/darwin@0.7.1 metaharness-darwin <verb>` directly (**verified working** — bypasses the pin); note the **MCP tool `metaharness_evolve` cannot reach 0.7.1** (it routes through the pinned `_darwin.mjs`). Treat 0.7.1 as *off the ruflo-tested path* until the 0.3→0.7 delta (possibly breaking) is smoke-checked. **A `/plugin` marketplace reload does NOT auto-bump the darwin pin** — re-check versions after each update.
