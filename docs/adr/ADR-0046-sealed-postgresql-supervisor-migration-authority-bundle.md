...
vectors and admits no `pg` package, driver, bridge or live client. The additive development-only V3 replay runs only `baseline-v1`, `baseline-v2`, `branch` and `final-where`, twice each in distinct fresh networkless anonymous-volume containers.
It accepts readiness only after PID 1 is exactly `postgres` and `pg_isready` succeeds, pins the historical V1/V2 receipts/runners byte-exact without invoking them, and remains outside every deployable closure under ADR-0048.
...
Commits `1e2d88d`, `7d5af51`, `0d5d09e` and `e37cce7` close only the Node oracle's lifecycle/control/deadline, command metadata, INSERT value/result and DDL-coupling slices. Fresh local exact Node 20.0.0 and 24.14.1 clean installs each pass all 715 tests. npm 9.6.4's platform detection requires `@rollup/rollup-linux-x64-gnu@4.63.1` as a direct development-only dependency for the pinned Linux evidence runner; runtime dependencies remain empty. The additive V3 replay retains historical V1/V2 receipts/runners byte-exact without invoking them and requires four profiles times two fresh networkless anonymous-volume containers with PID-1 `postgres` plus `pg_isready`; hosted run [`33636424967`](https://github.com/sparkling/semantic-fabric/actions/runs/33636424967) at exact `d0cc5fb938a1ff8b70859c19882934461fe23c5a` passes both exact-Node V3 lanes, but no hosted run receipt is tracked and no runtime/admission authority is granted. Oracle artifact SHA-256 is `422c6854e4dc8abdf24b30fbd4b6042b48a7acc8ca605365fcb6adc653dd3219`; the dependency-free public bundle remains byte-identical at 49,106 bytes and `90e21e7c0e3a45b66da55f0e8cf9c0a23b3fb82e805223922d81096e097f7c3a`. Supervisor catalogue-observation SELECT, PostgreSQL wire-transport/tag vectors and future separately packaged Rust-supervisor live execution remain open; Node execution expansion is closed.
