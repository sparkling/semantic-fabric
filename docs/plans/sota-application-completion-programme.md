...
1. close ADR-0047's remaining projection/oracle mutation and GitHub-hosted test-evidence gates, then build on ADR-0046's design: implement the immutable provisioning/migration bundle bytes and parsers, seed/manifest, driver-free runner and live catalogue/provisioning verifier, followed by ADR-0047's runtime `Plan`/live-observation brands and the dormant joined-row adapter before leases, resource fencing, outbox and independent deployment;
