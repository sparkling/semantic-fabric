...
1. close ADR-0047's remaining predicate/value/nullability/order/duplicate/array/`UNION ALL` and GitHub-hosted evidence gates (the eight class-branch deletions plus return-zero, one-atom omission/addition and count-neutral substitution are locally closed), then clarify ADR-0046's private executable, deadline and admission boundary and implement its atomic dormant bundle/parser/`Plan` slice before the live verifier, adapter, leases, resource fencing, outbox and independent deployment;
