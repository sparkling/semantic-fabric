...
| M0 — architectural truth and deterministic foundation | In progress | Backend-aware mapping/query/protocol receipts, exact-commit capture inputs, negative-only host admission, sealed one-shot observation, ELF/seccomp/bubblewrap checks, RFC 9162 checkpoint proofs, the private registration kernel/transaction/materializer, exact PostgreSQL catalogue/deparse oracle, and ADR-0047's independently replayed 4,059-record PUBLIC ACL candidate are implemented as non-authorizing slices. ADR-0046 now also has quoted-callable closure, a Linux-only component-walked held-descriptor reader and a fixed-root WeakMap-branded Plan that atomically binds manifest/catalogue/provisioning/in-memory seed/two SQL handles, returns only fresh `0001 -> seed -> 0002` bytes, and emits pathless receipt `2ff788e1…434d`; 661 private-service tests pass and the public five-export bundle remains `90e21e7c…f7c3a`. ADR-0042 through ADR-0047 remain proposed. Remaining M0 work includes ADR-0047 mutation/hosted evidence, driver/store/terminal runner and live verifier, PostgreSQL adapter/credentials, leases/fencing/outbox, independently administered supervision, actual-consumption runtime closure, final-FD/syscall/cgroup evidence, SBOM/reproducibility, production admission and controlled performance |
...
ADR-0042 through ADR-0047 remain proposed. ADR-0046 defines exact provisioning, manifest, seed, independent-pin, empty-state, whole-script and commit/cleanup semantics; its quoted-callable policy, private fixed reader and opaque runtime Plan are now implemented as a dormant representation slice without making provisioning bytes an oracle. The driver-free terminal runner, live catalogue/provisioning verifier, PostgreSQL adapter/writer connection, deployed signer, network, external administrator, lease/fence/outbox publisher, attempt/capture authority, and real supervisor events remain absent. ADR-0047 still requires its remaining mutation and GitHub-hosted evidence gates before activation.
...
1. close ADR-0047's remaining predicate/value/nullability/order/duplicate/array/`UNION ALL` and GitHub-hosted evidence gates, then implement ADR-0046's private driver/store/terminal runner with explicit deadlines and admission before the live verifier, adapter, leases, resource fencing, outbox and independent deployment;
