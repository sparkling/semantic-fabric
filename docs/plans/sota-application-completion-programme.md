...
|---|---|---|---|
| P1 | Recursive-path resource qualification | `5c379f6` computes an exact finite-pair fixed point beyond 256 and rejects unproved dialects; serving counts observable probe/open/pull attempts, not source rows or recursive iterations, and has no native cancellation contract | Charge recursive/source work to total `QueryBudget`; add live/native-cancellation qualification before backend admission |
...
| P1 | Governance covers only part of a request | One serving identity spans the absolute deadline, compiler/pool waits, observable probes/opens/pulls, semantic results and serializer writes; compiler CPU, database rows, recursive work/source cost, raw/conformance callers and native cancellation remain outside total governance | Extend the existing serving control into total `QueryBudget` for every backend |
...
packed-runtime resources change no identity, export, fetch/provider authority or application goal. Earlier 905-test evidence predates this overlay. The 2026-09-02 current-tree hermetic checkpoint passes 124 files/965 tests with two intentional skips after refreshing the five-path Rust source closure and controller attestation. The excluded seven-test ambient Ruflo collector has three passing fail-closed controls and four positive cases rejecting installed-package drift; the mutable source grants no authority, and a portable materializer remains open. The selected root is resolved once. V2 keeps its original physical-source `entryPath` and remains replayable but is not emitted for relocation; V3 binds the content-addressed source policy and private executed `/runtime` entry. It copies only exact status files and runs
