...
1. close ADR-0047's remaining scanner, runtime-admission and GitHub-hosted evidence gates for its reproduced PostgreSQL 16.15 test candidate, then build on ADR-0046's design: implement the immutable provisioning/migration bundle bytes and parsers, seed/manifest, driver-free runner and live catalogue/provisioning verifier, then add the dormant joined-row adapter before leases, resource fencing, outbox and independent deployment;
