...
| M0A — Rust application foundation | Decision-ready; gold lane in progress | ADR-0048 fixes a Rust-only product closure. ADR-0039 remains proposed: accepted ADR-0006 and immutable V1 `sf-cli` receipts prohibit a silent rename. Explicit adoption and a domain-separated V2 receipt precede the split; the product-mock gold/live vertical proceeds independently. Its admitted relational R2RML covers only one of 112 tables. Exact product tests, not harness scores, carry progress |
...
| M1–M6 — application completion | Unblocked; queued in parallel | Each product slice starts when its direct Rust preconditions are met and must pass its own executable QA gate; it does not wait for M0E operational infrastructure |
...
H0c is complete; M0A, M0E and eligible M1–M6 slices now proceed in parallel.
...
Its two-builder gate pre-registers roles, trust roots and run IDs, commits both
results before reveal, and requires byte identity. Missing, failed, mismatched,
selected, tiebroken or retried results fail; directional comparison is not agreement.

The corrected programme runs four parallel lanes:

1. **Rust product:** decide ADR-0039, add domain-separated V2 receipts, then split the binaries and run M1–M6 behind direct product gates; M0A is approximately 5–8 focused days after adoption.
2. **Gold/live vertical:** verify the canonical manifest, exact source checkout and live PostgreSQL independently, replay the one admitted R2RML map, and retain the explicit 111-table gap; SQL never proves container provenance. This saves an estimated 4–7 days of synthetic corpus work.
...
With three effective lanes, charter-complete application delivery remains an estimated 12–20 weeks, but the correction removes roughly 3–5 weeks of supervisor work from the front of the product critical path without removing features or final correctness gates.
