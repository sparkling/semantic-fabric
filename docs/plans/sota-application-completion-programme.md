...
1. **Rust product:** run independent trains for errors/budgets, bounded operators, federation contracts, runtime operations and packaging; integrate small verified commits at typed seams. ADR-0039 gates only its binary split, not M1–M6 behavior.
2. **Gold/live vertical:** complete. The KAT verifies all 139 canonical artifacts and every admitted byte in the 171-file source snapshot, then separately qualifies the live schema/query differential. It claims no source-worktree, image, build or source-to-container provenance and avoids 4–7 days of synthetic-corpus setup.
...
With five product trains plus two evidence lanes, the planning range is 6–10 weeks for M1–M6 feature completion and 10–16 weeks for charter-complete M7 release proof. Errors land before deadlines, shared interfaces before backend implementations, live/provider gates run independently, and immutable Cargo inputs/build caches may be reused only after digest validation. These changes shorten the critical path without dropping a feature or gate; measured slice velocity will replace the range after M2.
