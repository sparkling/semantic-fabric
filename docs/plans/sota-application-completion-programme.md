...
| M1–M6 — application completion | Active in parallel | Exact paths, branch-aligned fallback admission, bounded ordered-ASK admission, current router-level RFC 9457 failures, environment-secret references, one absolute deadline, linearizable serving-lane source/result/byte accounting, deterministic public deadline handoff with sticky internal first-cause accounting, prompt ASK termination, and an enforcing constraint/type-quarantined single-source binding with fail-closed base-source metadata preflight are integrated. Next: immutable structural/type digest generation, lease and atomic reload plus Direct Mapping lifecycle; in parallel: complete total QueryBudget, bounded operators, operations, then registry/federation. M0E does not block feature work |
...
- preserve the implemented serving slice: one linearizable identity spans the absolute
  deadline, compiler/pool waits, observable source work, semantic results and serializer
  bytes; representable expired handoffs are public `504` while internal first-cause
  accounting stays sticky, current router path/method/body failures are RFC 9457,
  and ordered ASK uses no global sort buffer while preserving slicing/existence;
...
- one deadline covers Axum router entry (after request-target parsing), parse,
  compile, acquire, execute and serialize;
