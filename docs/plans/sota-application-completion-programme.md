...
signed checkpoint parsing, and registration inclusion/consistency replay. Commit `f1a3a48` adds a sealed,
private, nonoperational decision kernel; `f604d0f` adds its dormant transaction coordinator. The coordinator
keeps write and exact-recovery roots disjoint, makes checkout acquisition allocation-free, captures checkout-local
cleanup before the sole `open()`, freezes adapter results, binds staged event and joined-row provenance to the
candidate and transaction snapshot, quarantines ambiguous terminal outcomes, and releases response bytes only after
literal commit. Valid unrelated global interleaving commits while forged original-registration provenance rolls back;
exact recovery cannot read head, run or staging authority. Service tests pass 204/204 under Node 20 and 24 with identical internal artifact `8a7ee550…2009b` and public bundle `90e21e7c…f7c3a`; the parent suite passes 829 tests with two expected skips, and four authority/manifest gates pass 33/33. Runtime npm audits and the scoped Ruflo service scan report zero findings; the whole-repository scan reports only two deliberate forbidden-import mutant strings in tests. Native Claude reviewed the transaction design, and final independent Codex adversarial, security and test-gap reviews found no P0/P1. ADR-0042 remains proposed: no PostgreSQL adapter, writer, signer, network, external administrator, lease/fence/outbox, runner, attempt, capture authority, or real supervisor event exists.
