...
- **Date:** 2026-08-26
- **Updated:** 2026-08-30
- **Decision records:** [ADR-0038](../adr/ADR-0038-sota-application-completion-programme.md) (accepted), [ADR-0039](../adr/ADR-0039-minimal-production-serving-artifact.md) (proposed artifact design), [ADR-0041](../adr/ADR-0041-manifest-bound-controlled-observational-evidence-capture.md) (proposed capture design), [ADR-0042](../adr/ADR-0042-witnessed-single-use-capture-supervisor-protocol.md) (proposed witnessed supervisor protocol), and [ADR-0043](../adr/ADR-0043-postgresql-supervisor-registration-state-and-dormant-adapter.md) (proposed PostgreSQL state/adapter design)
...
A known internal abort can trigger at most three fresh attempts after successful rollback/destruction; roots are snapshotted, the peer is consumed once, all decisions and staging are recomputed, and ambiguous commit or cleanup failure is never retried.
Service tests pass 239/239 under Node 20 and 24 with identical internal artifact `e3ad1e33…27f9` and unchanged public bundle `90e21e7c…f7c3a`; the parent suite passes 830 tests with two expected skips on both runtimes. Controller and service attestation files are byte-identical across runtimes (`afadb9e1…5d78` and `5e90d95d…ec06`), and the explicit `.cts`/`.cjs` launcher executes from a package-less directory in every CI run.
Scoped deep Ruflo runtime scans report zero findings; the whole-repository scan reports only two deliberate forbidden-import `execFileSync` mutant strings in tests. Native Claude and independent Codex adversarial, security and test-gap reviews found no P0/P1; Claude's P2 package-boundary execution gap was fixed before commit.
ADR-0042/0043 remain proposed: no PostgreSQL adapter, writer, signer, network, external administrator, lease/fence/outbox, runner, attempt, capture authority, or real supervisor event exists.
...
1. implement migrations and the operational joined-row PostgreSQL adapter behind the verified serializable transaction contract, then add leases, resource fencing and outbox and deploy under independent administration;
