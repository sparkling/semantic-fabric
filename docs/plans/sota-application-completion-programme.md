...
| M0E — evidence authority | Parallel; not product-blocking | Existing Node 20/24 supervisor work is frozen as a non-deployable executable oracle. Commits `0d5d09e`/`e37cce7` close INSERT values/results and structural DDL coupling. Exact Node 20.0.0 and 24.14.1 clean installs each pass all 706 tests; npm 9.6.4's Linux-native Rollup closure is pinned only as a development dependency. Artifact `422c6854…3219` keeps the dependency-free 49,106-byte public bundle unchanged at `90e21e7c…f7c3a`. Production bridge/store/runner work is Rust-only; controlled capture and exact-artifact evidence still gate authoritative performance and M7 |
...
| M1–M6 — application completion | Active in parallel | Exact paths, branch-aligned fallback admission, bounded ordered-ASK admission, redacted RFC 9457 failures, environment-secret references, an absolute deadline minted before Axum route/method dispatch, strict single-query/media-specific request admission, linearizable serving-lane source/result/byte accounting, deterministic public deadline handoff with sticky internal first-cause accounting, prompt ASK termination, and an enforcing constraint/type-quarantined single-source binding with fail-closed base-source metadata preflight are integrated. Next: immutable structural/type digest generation, lease and atomic reload plus Direct Mapping lifecycle; in parallel: complete total QueryBudget, bounded operators, operations, then registry/federation. Full Protocol and production admission remain open; M0E does not block feature work |
...
Commits `28addbc`/`c586973` add the exact catalogue/parser/deparse oracle; ADR-0047 adds the independently replayed 4,059-record PostgreSQL 16.15 PUBLIC candidate and mutation evidence. Commits `1e2d88d`/`7d5af51` freeze lifecycle and command metadata; `0d5d09e`/`e37cce7` add evidence-only INSERT value/result contracts and structural DDL coupling. Exact Node 20.0.0 and 24.14.1 clean installs each pass all 706 oracle-package tests. Artifact `422c6854…3219` keeps the public bundle at 49,106 bytes/`90e21e7c…f7c3a`, runtime dependencies empty and all authority flags false; the direct Linux-native Rollup pin is development-only. This Node lane is now an executable oracle only. Supervisor PostgreSQL catalogue-observation SELECT, PostgreSQL wire-transport/tag vectors, and the future separately packaged Rust supervisor's adapter/store/runner/live observations and admission remain Rust work; ADR-0047's remaining mutations/hosted evidence remain parallel evidence work.
...
If the second item is removed, the application charter must change from systems of record/cross-RDBMS federation to one source per deployment. This programme retains the accepted charter.
...
| P1 | Governance covers only part of a request | One serving identity spans the absolute deadline, compiler/pool waits, observable probes/opens/pulls, semantic results and serializer writes; compiler CPU, database rows, recursive SQL iterations/source cost, raw/conformance callers, a common source-native statement-cancellation contract and atomic/no-prefix post-`200` responses remain outside total governance | Extend the existing serving control into total `QueryBudget` for every backend |
...
neutral schema DTOs, atomic `QueryBudget` accounting with a governed serving
identity, and one immutable source/backend/capability/compiler-schema/constraint-
authority/cache binding. Next add stable digests/generations, drift/reload and
...
- preserve the implemented serving slice: one linearizable identity spans the absolute
  deadline minted in Tower `Service::call` after HTTP/request-target parsing and
  before Axum route/method dispatch, strict request admission, compiler/pool waits,
  observable source work, semantic results and serializer bytes; representable
  expired handoffs are public `504` while internal first-cause accounting stays
  sticky, and ordered ASK uses no global sort buffer while preserving slicing/existence;
- preserve strict request admission: exactly one GET/form query or one raw query;
  per-request raw cap `n`, checked form wire cap `3n+16` and decoded cap `n`;
  reject optional version/dataset/extra parameters and unsupported media without
  polling the application body; this is a subset, not full Protocol conformance;
- extend it into total `QueryBudget`: compiler CPU, database rows, source cost,
  recursive iterations, raw/conformance callers and stream-lifetime limits;
...
- one deadline covers Tower `Service::call` after request-target parsing but before
  Axum route/method dispatch, then admission, parse, compile, acquire, execute and serialize;
...
- exact result and byte caps work for every result format; total recursive/source
  work and atomic no-prefix failure remain required before admission.
...
packed-runtime resources change no identity, export, fetch/provider authority or application goal. Earlier 905-test evidence predates this overlay. The 2026-09-02 current-tree hermetic checkpoint passes 124 files/965 tests with two intentional skips after refreshing the expanded Rust source closure and controller attestation. The excluded seven-test ambient Ruflo collector has three passing fail-closed controls and four positive cases rejecting installed-package drift; the mutable source grants no authority, and a portable materializer remains open. The selected root is resolved once. V2 keeps its original physical-source `entryPath` and remains replayable but is not emitted for relocation; V3 binds the content-addressed source policy and private executed `/runtime` entry. It copies only exact status files and runs
