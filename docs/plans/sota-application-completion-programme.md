...
| M0 — architectural truth and deterministic foundation | In progress | Backend-aware v3 receipts bind all 87 SQLite and required-live PostgreSQL mapping outcomes; SQLite query/protocol baselines and the default `sf-cli` package closure have provider-free receipts; the first external current-`sf-cli` host observation was captured and replayed from clean `5a06eac`; performance capture/comparison machinery, exact capture-input attestation, a negative-only host gate, a same-UID run claim, claim-rooted exact-commit source materialization, a bounded runtime-linkage contract, descriptor-rooted sealed input holding, and `c8305c3`'s private one-shot exact-bubblewrap/sealed-copy loader observation exist; and `ad94cdb` replayed byte-identically in two clean checkouts. The one-shot is manual, in-memory and non-authorizing, with no product caller or receipt. External witnessing, positive runner admission, aggregate containment, complete tool/system/runtime closure, SBOM/reproducibility, production minimality/admission, and a controlled performance baseline remain open |
...
| P0 | Reproducibility closure is incomplete | `93ae3c2` tracks `Cargo.lock`; `374ca99` pins actions, images and selected tools; `31a1164` installs MetaHarness/Darwin from the npm lock; the package receipt closes resolution/features/edges; external observation `173d0698…51ca` binds one current binary plus observed build/link inputs; the private prepared one-shot now holds exact sealed sources and bubblewrap inode, applies bounded process/output controls and reproduces candidate loader resolution on an exact manual host, without receipt or authority | Accept a production collector design before promotion; bind bubblewrap's complete dynamic host closure, aggregate cgroup/process containment and final FD/syscall policy; add canonical receipt/witness/replay plus build-script, tool, linker and system provenance; produce SBOM and reproducibility/admission evidence; close hosted-runner, apt-transitive and release-toolchain residuals; repeat the complete M0 gate in two clean builders |
