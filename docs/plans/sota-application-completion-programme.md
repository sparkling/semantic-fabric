...
| M0 — architectural truth and deterministic foundation | In progress | Backend-aware mapping/query/protocol receipts, exact-commit capture inputs, negative-only host admission, sealed one-shot observation, ELF/seccomp/bubblewrap checks, RFC 9162 checkpoint proofs, the private registration kernel/transaction/materializer, exact PostgreSQL catalogue/deparse oracle, and ADR-0047's independently replayed 4,059-record PUBLIC ACL candidate are implemented as non-authorizing slices. ADR-0046 now also has quoted-callable closure, a Linux-only held-descriptor reader, opaque fixed-root Plan, descriptor-first receipt replay, unbranded descriptor-only capability capture and seven exact terminal singletons. Node 24 passes 677 private-service tests; exact Node 20 passes 17 changed/sealed gates, TypeScript and artifact replay; both pass 65 focused parent gates. Artifact `ce67c466…45e0` preserves public bundle `90e21e7c…f7c3a`. ADR-0042 through ADR-0047 remain proposed. Remaining M0 work begins with the reviewed 31-operation SQL/result catalogue, observation aliases/limits and script tag vectors, then adapter-owned store/Promise/protocol brands, runner/deadlines/live verifier, PostgreSQL credentials, ADR-0047 mutation/hosted evidence, leases/fencing/outbox, independent supervision, actual-consumption closure, final-FD/syscall/cgroup evidence, SBOM/reproducibility, production admission and controlled performance |
...
ADR-0042 through ADR-0047 remain proposed. ADR-0046's quoted-callable policy, fixed reader, opaque Plan, descriptor-first receipt, unbranded capability captures and exact terminal singletons are dormant non-authorizing representations. Its corrected lifecycle/no-op/Promise boundary is specified, but the exact statement/result catalogue remains the next design gate; the runner, protocol brands, live verifier, PostgreSQL adapter/writer connection, deployed signer, network, external administrator, lease/fence/outbox publisher, attempt/capture authority and real supervisor events remain absent. ADR-0047 still requires its remaining mutation and hosted evidence gates before activation.
...
1. freeze ADR-0046's exact 31-operation SQL/result catalogue, observation aliases/limits and 52/73 script tag vectors; then implement adapter-owned store/Promise/protocol brands, the bounded runner and PostgreSQL 16.15 live gates while closing ADR-0047's remaining mutation/hosted evidence in parallel, before leases, resource fencing, outbox and independent deployment;
