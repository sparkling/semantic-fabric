...
Commits `28addbc`/`c586973` add the exact catalogue/parser/deparse oracle; ADR-0047 adds the independently replayed 4,059-record PostgreSQL 16.15 PUBLIC candidate and mutation evidence. Commits `1e2d88d`/`7d5af51` freeze lifecycle and command metadata; `0d5d09e`/`e37cce7` add evidence-only INSERT value/result contracts and structural DDL coupling. Node 24 passes 705 oracle-package tests; exact Node 20 passes TypeScript, 23 focused tests and artifact replay. Artifact `715c72fd…0552` keeps the public bundle at 49,106 bytes/`90e21e7c…f7c3a`, dependencies empty and all authority flags false. This Node lane is now an executable oracle only. Supervisor PostgreSQL catalogue-observation SELECT, PostgreSQL wire-transport/tag vectors, and the future separately packaged Rust supervisor's adapter/store/runner/live observations and admission remain Rust work; ADR-0047's remaining mutations/hosted evidence remain parallel evidence work.
