...
| M0 — architectural truth and deterministic foundation | In progress | Backend-aware v3 receipts now bind all 87 SQLite and required-live PostgreSQL mapping outcomes; SQLite query/protocol baselines and the default `sf-cli` package closure have provider-free receipts; the first external current-`sf-cli` host observation was captured and replayed from clean `5a06eac`; performance capture/comparison machinery, exact capture-input attestation, a capability-minimal negative-only host gate, a non-authorizing same-UID run-claim reservation, claim-rooted exact-commit private source materialization, a bounded non-authorizing runtime-linkage plan/parser, and private descriptor-rooted sealed runtime-input holding exist; and `ad94cdb` replayed byte-identically in two clean checkouts. The holder has no production caller. Complete external claim witnessing, positive runner admission, tool/system and executed runtime closure, SBOM/reproducibility, production minimality/admission, and a controlled performance baseline remain open |
...
The current tranche adds a fail-closed **host-observed non-closure observation** for one freshly built current `sf-cli` executable. The first private `0600` external receipt, from clean `5a06eac`, replayed with 363 raw inputs, 357 canonical terminals and three one-hop HostSystem aliases; portable/host/receipt digests are `72ce37b4…9b9a`, `024fbbbd…3ad8` and `173d0698…51ca`. It is uncommitted, unpublished and noncanonical. CI tests only the parser/integration contract on mutable `ubuntu-24.04` and neither captures nor publishes. Linker-only alias authority binds alias topology and terminal bytes while generic authority remains symlink-rejecting; structured GNU-note parsing binds build-ID owner, type, size and digest. The producer still requires an exclusive, quiescent root/effective-UID builder. Same-principal/root ABA, linker time-of-use and path-resolution race resistance are explicitly unattested. The additive non-authorizing runtime-linkage contract describes a host-input-read-only, networkless, environment-cleared `ld.so --list` plan and canonicalizes strict bounded output. Commit `863a058` adds a private Linux holder that descriptor-resolves the discovered set, rejects untrusted ancestry/nested mounts/noncanonical aliases, snapshots exact source bytes into sealed close-on-exec memfds, reparses them by ELF role, and verifies canonical identities plus static `DT_NEEDED` provider equality/reachability with 22 focused authority/mutation tests. It exports no executor, receipt or descriptors and has no production caller: discovery remains prior and unauthorized, no production loader was invoked, no real current-artifact runtime set was captured, and no loader consumption or executed closure is proven. Configured tools and the final-link depfile do not attest complete tool execution or exclusive linker authorship; deliberate sealed-FD execution, process/output authority, build-script/system/`strace` closure, executed runtime resolution, VDSO bytes, receipt/replay, SBOM/provenance, reproducibility, production minimality and backend admission remain open.
