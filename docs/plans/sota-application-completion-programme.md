...
Commit `99fa2e1` adds the non-authorizing supervisor-registration verifier;
`92f5376` freezes RFC 8032 section 7.1 Test 1 with a public test-only seed that
is never a runtime key. Fixed claim/request fields and digest, acknowledgement
fields/digest, exact signing bytes/digest/signature, and exact canonical envelope
bytes/digest are pinned. The fixed SPKI and envelope traverse the production
verifier; wrong request/ack digest domains, empty or changed payloads, and changed
signatures fail. Receipt `97ca5d7…c36606f` rebuilt deterministically; 103 files
pass (742 tests, two expected skips), with independent native Codex and Claude
reviews. There is no production signer, transport, durable persistence/log,
external administration, lease, runner, attempt, state, capture authority,
rollback/fork/global-order proof, or real acknowledgement.
