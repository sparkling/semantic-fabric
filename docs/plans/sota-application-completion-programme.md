...
ADR-0042 through ADR-0047 remain proposed. ADR-0046 defines exact provisioning, manifest, seed, independent-pin, empty-state, whole-script and commit/cleanup semantics; its private fixed reader and opaque runtime Plan are now implemented as a dormant representation slice without making provisioning bytes an oracle. Quoted-identifier callable-policy closure, the driver-free terminal runner, live catalogue/provisioning verifier, PostgreSQL adapter/writer connection, deployed signer, network, external administrator, lease/fence/outbox publisher, attempt/capture authority, and real supervisor events remain absent. ADR-0047 still requires its remaining mutation and GitHub-hosted evidence gates before activation.
...
1. close ADR-0047's remaining predicate/value/nullability/order/duplicate/array/`UNION ALL` and GitHub-hosted evidence gates, harden ADR-0046's quoted callable semantics, then implement the private driver/store/terminal runner with explicit deadlines and admission before the live verifier, adapter, leases, resource fencing, outbox and independent deployment;
