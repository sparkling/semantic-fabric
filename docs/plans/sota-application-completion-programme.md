...
1. **Rust product:** freeze `SourceId`/`BoundMapping`, a shared `QueryControl`, and neutral schema DTOs first; then fan out budgets, one reusable bounded physical layer, federation, operations and packaging in isolated worktrees. ADR-0039 gates only its binary split.
2. **Gold/live vertical:** the existing KAT seals all 139 artifacts and 171 source files plus one live query differential. Next add a read-only 112-table/598-column schema differential without inferring the 111 missing mappings or claiming source/image provenance.
...
With five product trains plus two evidence lanes, the planning range remains 6–10 weeks for M1–M6 feature completion and 10–16 weeks for charter-complete M7 proof until measured velocity supports a narrower claim. One short contract-freeze gate replaces cross-lane merge churn; CI fans out service-free Rust, unit/SQLite, live providers, receipts, product-mock and promotion evidence, while Node 20/24 oracle gates run on protected-input changes plus scheduled drift. Exact-artifact proof is never cached away.
...
The first shared-contract gate moves neutral schema DTOs from the
`sf-mapping -> sf-sql` edge into `sf-core`, adds opaque `SourceId` plus a runtime
`BoundMapping` sidecar, and exposes pure `QueryControl` below `sf-serve`.
Decomposition follows characterization and targets only modules blocking lanes.
