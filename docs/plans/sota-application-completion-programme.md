...
| M0 — architectural truth and deterministic foundation | In progress | Backend-aware v3 receipts bind all 87 SQLite and required-live PostgreSQL mapping outcomes; SQLite query/protocol baselines and the default `sf-cli` package closure have provider-free receipts; the first external current-`sf-cli` host observation was captured and replayed from clean `5a06eac`; performance machinery, capture-input attestation, a negative-only host gate, a same-UID run claim, claim-rooted exact-commit source, bounded runtime linkage, sealed input holding, and `c8305c3`'s private one-shot observation exist; `805f413` adds a canonical private `authority=none` record and provider-free semantic reparse; `9282e60` checks a caller-supplied closed ELF tag/search/flag policy ID/five-source digest; `73e9864` binds a separately sealed exact late cBPF policy to the bubblewrap PID 1/reaper and loader child with a comparative native canary; `50adc0a` hashes and parses the same exact held bwrap bytes as `RootPie`; `b34b6d7` separately inventories controlled counterfactual name/path resolution; `99fa2e1` and `92f5376` freeze the signed registration seam; `7139b05`, `3ee0ed6`, and `d54518f` verify RFC 9162 inclusion/consistency and signed checkpoints; `f1a3a48` adds the sealed private registration decision kernel; `f604d0f` adds its dormant allocation-free checkout/transaction coordinator and exact-only recovery; `3e0ceab` adds bounded whole-transaction serializable-abort retry; the protected private materializer produces exact 201/409 rows without persistence authority; `28addbc` and `c586973` add the reviewed exact catalogue JSON, bounded parser/digest binder, hostile/semantic/limit KATs, and independent PostgreSQL 16.15 deparse oracle; `f192a8b` closes symbol-keyed capability escalation; ADR-0046 freezes the migration-bundle wire and terminal-state design; and ADR-0047 records an independently reproduced test-only PostgreSQL 16.15 PUBLIC ACL candidate. Its exact-platform-manifest-pinned 4,059-record, 860,988-byte fixture (`a108e05f…5be`), immutable V1 receipt and independent raw-SQL cross-check are reproduced by an owned networkless two-container runner using fresh anonymous volumes and `template0` databases. Additive V2 receipt `48d54b63…98e8` pins V1, an OID/attribute-aware candidate matrix, and 13,603 fresh-role effective-privilege `has_*` checks across six populated classes; FDW/server remain explicit zero classes. V1's raw oracle independently expands the direct ACL atoms, and V2 corroborates all 4,059 atoms. A protected test-only bounded scanner/one-parse reader and opaque `authority=none` byte brand add 92 hostile/limit/private-brand KATs plus five exact-fixture baseline tests. Unpublished developer-local exact Node 20/24 runs report V1/V2 plus an additive 12-mutant replay, 602/602 supervisor tests and 892 parent tests with two intentional skips; the parent harness closes Node 20 recursive-watcher teardown with explicit bounded directory watches and three agreeing unchanged-byte digests; no version-bound run receipt or passing GitHub-hosted evidence is carried by this change. A protected pure mutator, extracted fail-closed replay support and 17 focused KATs include hostile PostgreSQL escape-string/semicolon handling, component-walked held-descriptor reads, nonblocking FIFO rejection, exact process arguments, classified command failures, mutated container predicates and a 60/15/15-second session/probe/inspect hierarchy with 180 seconds of parent headroom. The delete-any-top-level-class-branch, return-zero, one-atom omission/addition and count-neutral-substitution subsets are closed locally; broader predicate/value/nullability/order/duplicate/array and `UNION ALL` replacement mutations, the sealed runtime parser/`Plan`, live-observation brands and admission remain open. ADR-0042 through ADR-0047 remain proposed; runtime still lacks immutable provisioning/migration bundle bytes and parsers, the seed/manifest, driver-free runner, live catalogue/provisioning verifier, database adapter/writer connection, and enforced resource fencing; `ad94cdb` replayed byte-identically in two clean checkouts. Receipt V1 remains byte-compatible and non-attesting. Positive independently administered supervision, runner admission, actual-consumption bwrap closure, final-FD/syscall evidence, aggregate containment, complete tool/system/runtime closure, SBOM/reproducibility, production admission, and controlled performance remain open |
...
Commits `28addbc`/`c586973` add the 232,822-byte exact catalogue (`e7ce3572…e69`; 9,125 nodes, 963 records), bounded pre-scan/one-parse contract verifier, caller-pinned digest binder, 123 KATs, and 85 PostgreSQL 16.15 deparse facts with zero fixture mismatch. Commit `f192a8b` makes shared capability admission reject hidden symbol keys before invocation. ADR-0047's exact-platform-manifest-pinned PostgreSQL 16.15 fixture has 4,059 records and 860,988 bytes (`a108e05f…5be`). Its immutable V1 receipt and owned networkless replay bind two fresh anonymous-volume `template0` runs, full profiles/results, independent raw-SQL transcripts and ownership-checked cleanup. Additive V2 receipt `48d54b63…98e8` pins V1, an OID/attribute-aware candidate matrix, and 13,603 effective-privilege checks by a fresh no-membership role across six populated classes; FDW/server remain explicit zero classes. V1's raw oracle independently expands the direct ACL atoms, and V2 corroborates all 4,059 atoms. A protected test-only scanner/reader and opaque non-authorizing fixture brand pass 97 focused tests. A protected pure mutator, extracted fail-closed replay support and 17 KATs drive two fresh networkless rollback-only runs in which FDW/server seeds make all eight classes positive, the original and normalized projections match the independent raw-catalogue bag, and all 12 enumerated branch-deletion/record-set mutants execute, parse, match their exact mutant outputs and fail the final full-bag comparison. Unpublished developer-local exact Node 20/24 runs report V1/V2 plus the 12-mutant replay, 602/602 service tests and 892 parent tests with two intentional skips; the parent harness closes Node 20 recursive-watcher teardown with explicit bounded directory watches and three agreeing unchanged-byte digests; these run results have no version-bound repository receipt, and passing GitHub-hosted evidence for this change remains open. Component-walked held-descriptor source confinement, nonblocking FIFO rejection, exact process arguments, classified command failures, mutated container predicates and a 60/15/15-second session/probe/inspect hierarchy leave 180 seconds beneath the parent child ceiling; CI's 120-minute limit exceeds the conservative 110-minute six-run/cleanup sum. The internal artifact remains `e296eabc2480d10b5bcb1ea8629e1855f82a1695891a0e96958de2b81e65b358` and the public bundle `90e21e7c…f7c3a`; broader predicate/value/nullability/order/duplicate/array and `UNION ALL` replacement mutations, the runtime `Plan`, live-observation brands and admission remain open.
Both npm audits are clean. The Ruflo deep generic scanner reports 12 high heuristics: three negative command-injection fixtures/assertions and nine static SQL/error-string patterns; line-level review found no untrusted command or SQL execution in this tranche. The repository-root upstream MetaHarness composite emits clean/info against the tracked posture file, but ADR-0037 treats it as inconclusive because the scanner does not inspect canonical `.mcp.json` or enforce runner isolation. After removing two ignored phantom `coding-harness@latest` registrations and their unused grants from developer-local child settings, the child composite emits `mcp-disabled/info`; this host-local gitignored cleanup is not promotion evidence. Canonical `.mcp.json` still has floating, unpinned, or network-capable coordination entries and the private Ruflo runner still uses `--share-net`; the project-owned configuration and runtime gates must close those separate M0 gaps. The ignored, untracked local two-task MetaHarness diagnostic suite verifies structurally at task hash `938531ad…a95` but is ineligible for Darwin/GEPA evolution.
ADR-0042 through ADR-0047 remain proposed. ADR-0046 defines exact provisioning, manifest, seed, independent-pin, empty-state, whole-script and commit/cleanup semantics; ADR-0047 specifies the pinned projection and records candidate/cross-check evidence on which bundle construction may build only after its remaining pre-sealing gates and GitHub-hosted evidence pass. Its post-bundle runtime `Plan` and live-observation gates then run against the immutable bundle without making provisioning bytes an oracle. Immutable provisioning/migration bundle bytes and parsers, the seed/manifest, driver-free runner, live catalogue/provisioning verifier, PostgreSQL adapter/writer connection, deployed signer, network, external administrator, lease/fence/outbox publisher, attempt/capture authority, and real supervisor events remain absent.
...
1. close ADR-0047's remaining pre-sealing §7 predicate/value/nullability/order/duplicate/array/`UNION ALL` and GitHub-hosted test-evidence gates (the eight top-level class-branch deletions plus return-zero, one-atom omission/addition and count-neutral substitution are locally closed), then build on ADR-0046's design: implement the immutable provisioning/migration bundle bytes and parsers, seed/manifest, driver-free runner and live catalogue/provisioning verifier, followed by ADR-0047's post-bundle runtime `Plan`/live-observation gate and the dormant joined-row adapter before leases, resource fencing, outbox and independent deployment;
...
or promotion gate. The genome verdict is ready with risk 0.21; the MCP scan and
threat model are clean. The OIA components ran clean with 2.89× measured parallel
speedup, but its legacy CLI could not persist through a live-WAL store and cannot
see the one receipt written through the native synchronized store. Drift from
history is therefore unproven; no bulk import, sidecar operation or store copy was
attempted. A fixed-seed Darwin search passed 11/12 with a statistically promoted,
zero-unsafe champion (fitness 0.88775, TPR 1, FPR 0.111111); only its upstream
≤2× efficiency cap failed at 2.267857×. It is not a provider-dollar ceiling. Product tests and the
