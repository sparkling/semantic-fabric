...
Commits `28addbc`/`c586973` add the 232,822-byte exact catalogue (`e7ce3572…e69`; 9,125 nodes, 963 records), bounded pre-scan/one-parse contract verifier, caller-pinned digest binder, 123 KATs, and 85 PostgreSQL 16.15 deparse facts with zero fixture mismatch. Commit `f192a8b` makes shared capability admission reject hidden symbol keys before invocation. Service tests pass 413/413 under Node 20 and 24 with identical internal artifact `0a5e1c11…eae7` and unchanged public bundle `90e21e7c…f7c3a`; the parent focused protected-capability suite passes on Node 24 and the prior full parent suite remains 889 tests with two expected skips on both runtimes.
Both npm audits and native MCP/threat-model scans are clean; the deep generic scanner reports only two deliberate forbidden-import `execFileSync` mutant strings whose tests prove rejection. The frozen two-task MetaHarness corpus verifies at task hash `938531ad…a95`; no Darwin/GEPA product evolution is authorized by that undersized corpus.
ADR-0042/0043/0044/0045/0046 remain proposed. ADR-0046 now closes exact provisioning, manifest, seed, independent-pin, empty-state, whole-script and commit/cleanup semantics; migration bytes/parsers, the driver-free runner, live catalogue/provisioning verifier, PostgreSQL adapter/writer connection, deployed signer, network, external administrator, lease/fence/outbox publisher, runner, attempt, capture authority, and real supervisor events remain absent.
...
1. derive immutable migration SQL/manifest from the reviewed catalogue, implement the driver-free runner and live catalogue/provisioning verifier, then add the dormant joined-row adapter and exact-pinned PostgreSQL 16.15 required-live evidence before leases, resource fencing, outbox and independent deployment;
