...
#![cfg(test)]
    ...
    use super::*;
    use sf_core::ir::{LogicalSource, Segment, TermMap, TermSpec, Template};
    ...
    fn prune_drops_contradictory_equalities() {
        ...
        b.where_conds.push(SqlCond::Cmp(ColRef::new(0, "id"), CmpOp::Eq, "1".into()));
        b.where_conds.push(SqlCond::Cmp(ColRef::new(0, "id"), CmpOp::Eq, "2".into()));
        let out = run(vec![b], &[], &CascadeCtx::default());
        assert!(out.is_empty(), "contradictory =1 ∧ =2 must prune the branch");
    }
    ...
    fn self_join_not_eliminated_on_nullable_unique_key() {
        ...
        let b = Branch {
            ...
            opts: Vec::new(),
            bindings: BTreeMap::new(),
            where_conds: vec![SqlCond::ColEq(ColRef::new(0, "email"), ColRef::new(1, "email"))],
            distinct: false,
            limit: None,
            offset: 0,
            path: None,
        ...
        let out = run(vec![b], std::slice::from_ref(&ts), &CascadeCtx::default());
        assert_eq!(out[0].core.len(), 2, "nullable unique key ⇒ self-join preserved");
    ...
    fn fk_branch() -> Branch {
        let mut b = Branch {
            ...
            opts: Vec::new(),
            bindings: BTreeMap::new(),
            where_conds: vec![SqlCond::ColEq(ColRef::new(0, "rid"), ColRef::new(1, "route_id"))],
            distinct: false,
            limit: None,
            offset: 0,
            path: None,
        ...
        b.bindings
            .insert("route".into(), iri_template_binding(1, "http://ex/route/", "route_id"));
        b
    }
    ...
    fn fk_pk_join_eliminated_on_notnull_fk_to_pk() {
        ...
        assert!(!refs_parent, "no surviving reference to the dropped parent alias");
    }
    ...
    fn fk_pk_not_eliminated_when_parent_other_column_used() {
        ...
        let mut schema = gtfs_schema();
        schema[1].columns.push(Column::new("long_name", "text", false));
        let mut b = fk_branch();
        b.bindings.insert("rname".into(), col_binding(1, "long_name"));
        let out = run(vec![b], &schema, &CascadeCtx::default());
        assert_eq!(out[0].core.len(), 2, "parent contributes a non-PK column ⇒ kept");
    }
    ...
    fn selection_pushdown_flattens_and_hoists() {
        ...
        assert!(matches!(b.where_conds[1], SqlCond::ColEq(..)), "join equality after");
    }
    ...
    fn distinct_removed_when_projected_key() {
        ...
        let ctx = CascadeCtx { distinct: true, project: None };
    ...
    fn distinct_kept_when_key_not_projected() {
        ...
        let ctx = CascadeCtx { distinct: true, project: Some(&proj) };
        let out = run(vec![b], std::slice::from_ref(&emp_pk()), &ctx);
    ...
    fn distinct_kept_on_join() {
        ...
        let ctx = CascadeCtx { distinct: true, project: None };
        let out = run(vec![b], std::slice::from_ref(&emp_pk()), &ctx);
        assert!(out[0].distinct, "join ⇒ DISTINCT preserved (no uniqueness proof)");
    }
