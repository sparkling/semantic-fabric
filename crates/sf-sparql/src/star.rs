...
pub fn rewrite_query(query: &Query) -> Result<(Query, StarEnv)> {
    ...
    let rewritten = match query {
        ...
        // ([`substitute_construct_template`]) the relaxation's soundness
        // argument does not cover, and DESCRIBE/ASK do not share SELECT's
        // "the query's only consumer is the projection" shape.
        Query::Select {
            dataset,
            pattern,
            base_iri,
...
/// [`rewrite_union`]'s disagreement check (and [`decompose_column`]'s mixed-
/// VALUES-column check) exist because [`StarEnv`] is a WHOLE-QUERY map: a
/// variable's composed-ness, once resolved, is looked up STATICALLY (one
/// answer for the whole query) by exactly three consumers —
/// [`rewrite_and_check_composed`] (SUBJECT/PREDICATE/OBJECT/isTRIPLE,
/// `=`/`sameTerm`) and [`substitute_composed_term`] (CONSTRUCT templates).
...
/// ([`apply_composed_bindings`]'s `None`-on-absence doc comment; each
/// top-level `Plan` branch executes as its OWN SQL statement and is
...
/// `rewrite_pattern` / `rewrite_union(top_level: false)` / `rewrite_values`,
/// the original 501. `CONSTRUCT`/`DESCRIBE`/`ASK` never call this function
/// at all (see [`rewrite_query`]'s own doc comment).
fn rewrite_top_level_pattern(
    gp: &GraphPattern,
    n: &mut usize,
    env: &mut StarEnv,
) -> Result<GraphPattern> {
...
/// [`decompose_column`] itself checks, computed here BEFORE decomposition so
/// the split can run instead of the 501. Multi-column VALUES is left
/// entirely alone (falls through to the ordinary, unchanged
/// [`rewrite_values`]) — row-partitioning multiple independently-mixed
/// columns at once is a harder problem this ledger item does not attempt.
...
/// `type_complexity` lint (mirrors [`ComposedComponents`]'s own reason for
/// existing).
...
fn partition_values_by_triple_shape(
    bindings: &[Vec<Option<GroundTerm>>],
...
/// mixed-VALUES arms; the ordinary recursive call from [`rewrite_pattern`]
/// always passes `false`, so a `Union` reached any other way (nested under a
/// FILTER/BIND/further join/GROUP BY/ORDER-BY-expression, or as an inner arm
/// of a larger Union spine) keeps the original, unconditional check.
fn rewrite_union(
    left: &GraphPattern,
    right: &GraphPattern,
    n: &mut usize,
